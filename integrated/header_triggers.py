...
ti_on_switch_to_map           = -75.0 # Patch 1.165+. In simple triggers will fire after switching to map. (In Viking Conquest it changes weather).
...
ti_on_missile_hit             = -52.0 # Missile has hit something. Not all hits will be registered on client-side in multiplayer missions.
    ...
    # trigger param #2 = hit object type:
        ...
        # 9 = neutral agent
        # 10 = under water
    # pos1 = Missile moves by defined array of positions. Returned <pos1> is position from this array
    #        after missile intersects with collision mesh. To get the exact position use <cast_ray> operation.
...
ti_on_shield_hit              = -80.0 #
    ...
    #     WSE param 6 = item modifier
    #     WSE param 7 = missile item modifier
...
ti_tab_pressed                = -21.0 # Player has clicked `Leave location/Retreat` game key ('Tab' by default). Doesn't fire during dialogs.
...
ti_on_agent_killed_or_wounded = -26.0 # Agent has been defeated in battle (killed or wounded)
    ...
    # If (set_trigger_result) with non-zero parameter is used in the code, it will override the agent's fate.
    # Trigger result: 0 = do not alter the result, 1 = force kill, 2 = force wound.
    # <trigger_param_3> will not change value in the second and subsequent <ti_on_agent_killed_or_wounded> triggers.
    # <agent_is_wounded> operand will change the returning value after all <ti_on_agent_killed_or_wounded> triggers finished.
    # If horse killed somebody shows it's rider as killer/attacker.
...
ti_on_agent_hit               = -28.0 # Agent has been damaged with a weapon attack. Does not fire on client side in multiplayer missions.
    # trigger param 1 = damage inflicted agent_id (receiver)
    # trigger param 2 = attacker agent_id (dealer)
    # trigger param 3 = inflicted damage
    # trigger param 4 = hit bone
    # trigger param 5 = item_id of missile used to attack (if attack was with a ranged weapon)
    #     WSE param 6 = raw damage (before being soaked by armor)
    #     WSE param 7 = item modifier
    #     WSE param 8 = missile item modifier
    #     WSE param 9 = damage type
    #            reg0 = weapon item_id
    #        WSE reg1 = hit bone
    #            pos0 = position of the hit area, rotation fields contain the direction of the blow
    # Notes:
    #       If (set_trigger_result) is equal or greater than zero, it will override the inflicted damage.
    #       Trigger fire even when 0 damage was inflicted.
    #       If multiple triggers changed damage only the last <set_trigger_result> will be applied.
    #       If <agent_set_hit_points> sets to 0 inside trigger block, agent will be killed after trigger finished even when result damage is none.
...
ti_on_agent_mount             = -55.0 # An agent has mounted a horse. Has access to horse <agent_get_horse> inside trigger.
    # trigger param 1 = agent_id
    # trigger param 2 = horse agent_id

ti_on_agent_dismount          = -56.0 # An agent has dismounted a horse. Has access to horse <agent_get_horse> inside trigger.
    # trigger param 1 = agent_id
    # trigger param 2 = horse agent_id
...
ti_on_scene_prop_is_animating       = -45.0 # Called each frame when a scene prop is animating after mission timer triggers.
    # trigger param 1 = prop instance_id reference
...
ti_scene_prop_deformation_finished  = -76.0 # Deformation sequence has been completed for a scene prop.
    # This trigger can be used to retrieve new positions of agents for the next frame.
    # trigger param 1 = prop instance_id reference
...
ti_on_scene_prop_intersect_with_agent = ti_on_scene_prop_stepped_on
# Trigger Param 1: agent no
